    ...
    <div className="fixed bottom-0 left-0 right-0 bg-red-50 border-t border-red-200 py-2 px-4 z-50">
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-4 text-sm">
        <div className="flex items-center gap-2 text-red-900">
          ...
          <span className="font-semibold">{t('crisisFooter.title', 'Crisis?')}</span>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4 text-red-800">
          ...
          >
            ...
            {t('crisisFooter.call', 'Call 988')}
          </a>
          <span className="hidden sm:inline">|</span>
          <span className="hidden sm:inline">{t('crisisFooter.text', 'Text HOME to 741741')}</span>
          <span className="hidden sm:inline">|</span>
          <span className="text-xs text-red-700">{t('crisisFooter.note', 'Not medical advice • Not therapy')}</span>
        </div>
      </div>
    </div>
  );
