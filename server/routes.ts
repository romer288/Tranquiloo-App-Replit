...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/therapist/patient/:id/analytics', async (req, res) => {
    try {
      ...
      res.json({
        patientName: 'Patient X', // Anonymized for privacy
...
Context: ${context}
Patient Data: Anonymized as "Patient X" for privacy (HIPAA readiness in progress).
...
- Consider group therapy options
  ...
  // Patient-Therapist connection endpoint (working toward HIPAA readiness)
  app.post('/api/therapist-connections', async (req, res) => {
    try {
      ...
      console.log('📝 Creating therapist connection (HIPAA readiness in progress):', {
      ...
      const sendConnectionEmail = async (connectionId: string) => {
        ...
        const emailContent = `
          ...
          <p>This connection requires your explicit acceptance. Patient details will only be revealed after you accept the connection. We are working toward HIPAA readiness; please avoid sharing PHI until formal agreements are in place.</p>

          <h3>Next Steps:</h3>
          <p>Log into your therapist dashboard to review this connection request and make your decision.</p>
          <p><a href="${appUrl}/therapist-login?redirect=/therapist-dashboard" style="display: inline-block; background: #3b82f6; color: white; padding: 10px 20px; text-decoration: none; border-radius: 6px; margin: 10px 0;">View Connection Request</a></p>

          <hr>
          <p><small>This email was generated by the Tranquil Support app. We are working toward HIPAA readiness; please avoid sharing PHI until formal agreements are in place.</small></p>
        `;
      ...
      await sendConnectionEmail(connection.id);

      res.json({ 
        success: true, 
        message: 'Connection request sent successfully - therapist will be notified',
