...
var dbPath, sqliteDb, db2, sqliteDatabase;
var init_db = __esm({
  "server/db.ts"() {
    "use strict";
    dotenv.config();
    process.env.NODE_TLS_REJECT_UNAUTHORIZED = "0";
    dbPath = "./data/tranquiloo.db";
    console.log("[DB] Using SQLite database at", dbPath);
    try {
      mkdirSync(dirname(dbPath), { recursive: true });
    } catch {
    }
    sqliteDb = new Database2(dbPath);
...
var ensureTreatmentPlanTable, ensureUserGoalsSourceColumn, logTreatmentPlanDebug, TREATMENT_PLAN_GOAL_SOURCE, DatabaseStorage, storage;
var init_storage = __esm({
  "server/storage.ts"() {
    ...
    ensureUserGoalsSourceColumn = () => {
      try {
        const columns = sqliteDatabase.prepare("PRAGMA table_info(user_goals);").all();
    ...
    DatabaseStorage = class {
      ...
      async upsertTreatmentPlan(patientId, plan) {
        ...
        const existing = sqliteDatabase.prepare("SELECT id FROM treatment_plans WHERE patient_id = ? LIMIT 1").get(patientId);
        if (existing?.id) {
          sqliteDatabase.prepare("UPDATE treatment_plans SET plan = ?, updated_at = ? WHERE patient_id = ?").run(planJson, timestamp2, patientId);
          logTreatmentPlanDebug("Updated existing plan for patient", patientId);
        } else {
          sqliteDatabase.prepare("INSERT INTO treatment_plans (id, patient_id, plan, created_at, updated_at) VALUES (?, ?, ?, ?, ?)").run(randomUUID(), patientId, planJson, timestamp2, timestamp2);
